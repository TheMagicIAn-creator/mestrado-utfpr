...
def test_published_json_contracts_are_strict_json():
    ...
    assert reliability["schema_version"] == 6
    ...
    assert fmeca["status"] == "awaiting_user_fmeca"
    assert fmeca["calculation_enabled"] is False
    assert all(item["npr"] is None for item in fmeca["components"])
