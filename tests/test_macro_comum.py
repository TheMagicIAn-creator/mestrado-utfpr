...
def _resultado(
    nome: str, auc_ct: float, det_ct: float, auc_ig: float, det_ig: float, fp: float = 1.1
) -> dict:
...
def test_tabela_enxuta_tem_5_colunas_e_uma_linha_por_metodo_falha():
    tab = tabela_enxuta(
        [
            _resultado("Proposto", 0.99, 1.0, 0.94, 0.86),
